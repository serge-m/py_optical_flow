...
class MultiscaleOF:
    def __init__(self, I0, I1, pyrlevels, pyrfactor, train_function_args=dict(rate=0.1, num_steps = 1000, alpha = 0.1), class_train_function = TrainFunctionTV):
        ...
        self.I0pyr = construct_image_pyramid(I0, self.pyrlevels, self.pyrfactor)
        self.I1pyr = construct_image_pyramid(I1, self.pyrlevels, self.pyrfactor)
        self.warps = 10
        self.u = None
        self.v = None
    ...
    def process(self):
        for level in range(self.pyrlevels - 1, -1, -1):
            if level == self.pyrlevels - 1:
                ...
                rescale_v, rescale_u = numpy.array([M, N], dtype='float32') / self.I0pyr[level + 1].shape
